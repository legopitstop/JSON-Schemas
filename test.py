...
logging.basicConfig(
    ...
    datefmt="%I:%M:%S",
    handlers=[
        logging.FileHandler(os.path.join(PATH, "latest.log"), mode="w"),
...
class ExtendedRefResolver(jsonschema.RefResolver):
    ...
    def resolve_local(self, path: str):
        ...
        if path == "common.json":
            path = os.path.join(PATH, "schemas", "bedrock", "common.json")
        schema = {}
        try:
        ...
        for key in obj:
            if key == "$ref":
                ...
                if ref.startswith("./"):
                    obj["$ref"] = (
                        "file:///"
                        + os.path.join(PATH, "schemas", "bedrock").replace(" ", "%20")
                        + ref.replace("../", "/").replace("./", "/")
                    )
            else:
                fix(obj[key])
        ...
        if os.path.isfile(fp):  # Only open if path is a file.
            ...
            with open(fp, "r") as r:
                ...
                try:
                    ...
                    logging.warning(
                        f"{name} ValidationError '{filename}': {err.message} at {path}"
                    )
                    res = False
...
def callback1():
    if args.all or args.resoruce or args.animation_controllers:
        files = glob.glob(
            PATH + "/tests/vanilla_RP/animation_controllers/**", recursive=True
        )
        if test("animation_controllers", files):
            logging.info("Animation Controllers: Passed")
        else:
            logging.warning("Animation Controllers: Failed")


def callback2():
    if args.all or args.resoruce or args.animations:
        files = glob.glob(PATH + "/tests/vanilla_RP/animations/**", recursive=True)
        if test("animations", files):
            logging.info("Animations: Passed")
        else:
            logging.warning("Animations: Failed")


def callback3():
    if args.all or args.resoruce or args.attachables:
        files = glob.glob(PATH + "/tests/vanilla_RP/attachables/**", recursive=True)
        if test("attachables", files):
            logging.info("Attachables: Passed")
        else:
            logging.warning("Attachables: Failed")
...
def callback6():
    if args.all or args.resoruce or args.client_entities:
        ...
        if test("client_entity", files):
            logging.info("Client Entity: Passed")
        else:
            logging.warning("Client Entity: Failed")
...
def callback7():
    if args.all or args.resoruce or args.flipbook_textures:
        if test(
            ...
            [
                os.path.join(
                    PATH, "tests", "vanilla_RP", "textures", "flipbook_textures.json"
                )
            ],
        ):
            logging.info("Flipbook Textures: Passed")
        else:
            logging.warning("Flipbook Textures: Failed")
...
def callback8():
    if args.all or args.resoruce or args.fogs:
        ...
        if test("fogs", files):
            logging.info("Fogs: Passed")
        else:
            logging.warning("Fogs: Failed")


def callback9():
    if args.all or args.resoruce or args.models:
        files = glob.glob(PATH + "/tests/vanilla_RP/models/**", recursive=True)
        if test("models", files):
            logging.info("Models: Passed")
        else:
            logging.warning("Models: Failed")
...
def callback11():
    if args.all or args.resoruce or args.render_controllers:
        files = glob.glob(
            PATH + "/tests/vanilla_RP/render_controllers/**", recursive=True
        )
        if test("render_controllers", files):
            logging.info("Render Controllers: Passed")
        else:
            logging.warning("Render Controllers: Failed")
...
def callback12():
    if args.all or args.resoruce or args.sound_definitions:
        if test(
            ...
            [
                os.path.join(
                    PATH, "tests", "vanilla_RP", "sounds", "sound_definitions.json"
                )
            ],
        ):
            logging.info("Sound Definitions: Passed")
        else:
            logging.warning("Sound Definitions: Failed")
...
def callback13():
    if args.all or args.resoruce or args.terrain_texture:
        if test(
            ...
            [
                os.path.join(
                    PATH, "tests", "vanilla_RP", "textures", "terrain_texture.json"
                )
            ],
        ):
            logging.info("Terrain Texture: Passed")
        else:
            logging.warning("Terrain Texture: Failed")
...
def callback14():
    if args.all or args.resoruce or args.item_texture:
        if test(
            ...
            [
                os.path.join(
                    PATH, "tests", "vanilla_RP", "textures", "item_texture.json"
                )
            ],
        ):
            logging.info("Item Texture: Passed")
        else:
            logging.warning("Item Texture: Failed")


def callback15():
    if args.all or args.resoruce or args.texture_set:
        files = glob.glob(
            PATH + "/tests/vanilla_RTX/textures/**/*.texture_set.json", recursive=True
        )
        if test("texture_set", files):
            logging.info("Texture Set: Passed")
        else:
            logging.warning("Texture Set: Failed")
...
def callback16():
    if args.all or args.resoruce or args.ui:
        ...
        if test("ui", files):
            logging.info("UI: Passed")
        else:
            logging.warning("UI: Failed")
...
def callback17():
    if args.all or args.resoruce or args.camera_entities:
        ...
        if test("cameras", files):
            logging.info("Camera Entities: Passed")
        else:
            logging.warning("Camera Entities: Failed")


def callback18():
    if args.all or args.resoruce or args.font_metadata:
        files = glob.glob(
            PATH + "/tests/vanilla_RP/font/font_metadata.json", recursive=True
        )
        if test("font_metadata", files):
            logging.info("Font Metadata: Passed")
        else:
            logging.warning("Font Metadata: Failed")


def callback19():
    if args.all or args.resoruce or args.materials:
        files = glob.glob(
            PATH + "/tests/vanilla_RP/materials/*.material", recursive=True
        )
        if test("materials", files):
            logging.info("Materials: Passed")
        else:
            logging.warning("Materials: Failed")


def callback20():
    if args.all or args.resoruce or args.sounds:
        files = glob.glob(PATH + "/tests/vanilla_RP/sounds.json", recursive=True)
        if test("sounds", files):
            logging.info("Sounds: Passed")
        else:
            logging.warning("Sounds: Failed")


def callback21():
    if args.all or args.resoruce or args.music_definitions:
        files = glob.glob(
            PATH + "/tests/vanilla_RP/sounds/music_definitions.json", recursive=True
        )
        if test("music_definitions", files):
            logging.info("Music Definitions: Passed")
        else:
            logging.warning("Music Definitions: Failed")


def callback22():
    if args.all or args.resoruce or args.pieces:
        files = glob.glob(PATH + "/tests/vanilla_RP/pieces/**/*.json", recursive=True)
        if test("pieces", files):
            logging.info("Pieces: Passed")
        else:
            logging.warning("Pieces: Failed")
...
def callback36():
    if args.all or args.behavior or args.loot_tables:
        ...
        if test("loot_tables", files):
            ...
            logging.warning("Loot Tables: Failed")
...
def callback38():
    if args.all or args.behavior or args.recipes:
        ...
        if test("recipes", files):
            ...
            logging.warning("Recipes: Failed")
...
def callback41():
    if args.all or args.behavior or args.trading:
        ...
        if test("trading", files):
            ...
            logging.warning("Trading: Failed")
...
def callback43():
    if args.all or args.behavior or args.cameras:
        ...
        if test("cameras", files):
            ...
            logging.warning("Cameras: Failed")
...
def callback44():
    if args.all or args.behavior or args.behavior_tree:
        ...
        if test("behavior_tree", files):
            ...
            logging.warning("Behavior Tree: Failed")
